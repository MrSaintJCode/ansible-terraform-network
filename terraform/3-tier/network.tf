...
###################### Database Subnet(s) ######################
# Database Subnet - 1a
resource "aws_subnet" "workspace_database_subnet_1a" {
  vpc_id            = aws_vpc.workspace_vpc.id
  cidr_block        = var.db_subnet_1a
  availability_zone = "${var.region}a"
  #map_public_ip_on_launch = false
  tags = {
    "Name"         = "${terraform.workspace}-db_sub_1a"
    "Env"          = "${terraform.workspace}"
    "Project"      = "${terraform.workspace}"
    "DeployedWith" = "terraform"
  }
...
# Database Subnet - 1b
resource "aws_subnet" "workspace_database_subnet_1b" {
  vpc_id            = aws_vpc.workspace_vpc.id
  cidr_block        = var.db_subnet_1b
  availability_zone = "${var.region}b"
  #map_public_ip_on_launch = false
  tags = {
    "Name"         = "${terraform.workspace}-db_sub_1b"
    "Env"          = "${terraform.workspace}"
    "Project"      = "${terraform.workspace}"
    "DeployedWith" = "terraform"
  }
